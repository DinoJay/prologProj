...
add_to_schedule_list_wrapper([], NewSol, NewSol).
add_to_schedule_list_wrapper([PC | Rest], Sol, Res):-
  add_to_schedule_list(PC, Sol, NewSol),
  add_to_schedule_list_wrapper(Rest, NewSol, Res).
...
find_optimal(Sol):-
  depSort(TaskList),
  find_one(TaskList, [], Sol), !.

find_one([], ScheduleList, ScheduleList).

find_one([Task|Tasks], ScheduleList, ResSchedule):-
  % binds only, if stated more solutions possible (?)
  findall(process_cost(Task, C, Time),
    process_cost(Task, C, Time),
  ProcCosts),
  add_to_schedule_list_wrapper(ProcCosts, ScheduleList, NewScheduleList),
  find_one(Tasks, NewScheduleList, ResSchedule).
...
maxETList([X], X) :- !.  maxETList([process_cost(T1, C1, Ti1), process_cost(T2, C2, Ti2)|Tail], Res):-
    ( Ti1 > Ti2 ->
        minProcList([process_cost(T2, C2, Ti2)|Tail], Res)
    ;
        minProcList([process_cost(T1, C1, Ti1)|Tail], Res)
...
make_graph_weight(Graph):-
  findall(T, make_edge_weight(T), Ts),
  (Ts = [] ->
    fail
  ;
    flatten(Ts, Fl),
    vertices_edges_to_ugraph([], Fl, Graph)
    %top_sort(L, Sorted), !
  ).
...
make_edge_pc([process_cost(EnablingTask, C0, ET0)-process_cost(DepTask, C1, ET1)]):-
  process_cost(EnablingTask, C0, ET0),
  process_cost(DepTask, C1, ET1),
  depends_on(DepTask, EnablingTask, _).

make_edge_weight([tuple(process_cost(EnablingTask, C0, ET0), S)-tuple(process_cost(DepTask, C1, ET1), 0)]):-
  process_cost(EnablingTask, C0, ET0),
  process_cost(DepTask, C1, ET1),
  depends_on(DepTask, EnablingTask, _),
  (not(depends_on(EnablingTask, Pred, _)) ->
    S is 0
  ;
    S is -10000
  ).
...
make_edge([EnablingTask-DepTask]):-
  process_cost(EnablingTask, _, _),
  process_cost(DepTask, _, _),
  depends_on(DepTask, EnablingTask, _).
...
create_graph([process_cost(T, C, ET)|Tail], Edges0, Res):-
  %process_cost(T, C, ET),
  %append(
  ...
  findall(PC-process_cost(T, C, ET),
    make_edge([PC-process_cost(T, C, ET)]),
  Edges),

  create_graph(Tail, Edges, Res).
...
search_df_wrapper(Graph, [], Tmp1, 0).
...
find_DSC(Res):-
  make_graph_weight(Graph),
  top_sort(Graph, SortedNodes),
  find_DSC(Graph, SortedNodes, Res).

find_DSC(NewGraph, [], NewGraph).
find_DSC(Graph, [Node|Rest], Res):-
  edges(Graph, Edges),
  neighbours(Node, Graph, NL),
  incr_dist(Node, NL, [], Vertices),
  del_vertices(Graph, NL, ComplGraph),
  add_vertices(ComplGraph, Vertices, NewGraph0),
  add_edges(NewGraph0, Edges, NewGraph1),
  find_DSC(NewGraph1, Rest, Res).

incr_dist(_, [], Tmp, Tmp).
incr_dist(tuple(process_cost(T0, C0, ET0), W0), [tuple(process_cost(T1, C1, ET1), W1) |Rest], Tmp, Res):-
  channel(C0, C1, Lat, Bwidth),
  depends_on(T1, T0, Data),
  Com = Lat + Data/Bwidth,
  W1New is W0+Com,
  ( W1 < W1New ->
      append([tuple(process_cost(T1, C1, ET1), W1New)], Tmp, Tmp1),
      incr_dist(tuple(process_cost(T0, C0, ET0), W0), Rest, Tmp1, Res)
    ;
      incr_dist(tuple(process_cost(T0, C0, ET0), W0), Rest, Tmp, Res)
  ).
